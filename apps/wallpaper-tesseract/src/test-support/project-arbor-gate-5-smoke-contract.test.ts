...
interface ProjectArborGate5SmokeEvidence {
...
describe("Project Arbor Gate 5 smoke evidence", () => {
  it("validates a fresh Gate 5 browser smoke file when provided", () => {
    const evidencePath = process.env.PROJECT_ARBOR_GATE_5_SMOKE_EVIDENCE;
    ...
    const evidence = JSON.parse(readFileSync(resolvedPath, "utf8")) as ProjectArborGate5SmokeEvidence;
    expect(validateProjectArborGate5SmokeEvidence(evidence)).toEqual([]);
  });

  it("rejects missing Arbor Scene subtree and fullscreen evidence", () => {
    expect(validateProjectArborGate5SmokeEvidence({
      kind: "project-arbor-gate-5-smoke",
      passed: true,
      validationErrors: [],
      consoleErrors: [],
      scene: {
  ...
  it("rejects Scene snapshots whose render viewport does not fill the Scene root", () => {
    const errors = validateProjectArborGate5SmokeEvidence(createValidEvidence({
  ...
  it("rejects fullscreen resize evidence that keeps stale desktop rects", () => {
    ...
    const errors = validateProjectArborGate5SmokeEvidence(evidence);
...
function validateProjectArborGate5SmokeEvidence(
  evidence: ProjectArborGate5SmokeEvidence
): string[] {
  const errors: string[] = [];
  if (evidence.kind !== "project-arbor-gate-5-smoke") errors.push("kind must be project-arbor-gate-5-smoke.");
...
function validateCloseReopenEvidence(closeReopen: Record<string, unknown> | null, errors: string[]): void {
  ...
  if (iterations.length < 2) {
    errors.push("close/reopen evidence must contain at least two iterations.");
  }
...
function createValidEvidence(snapshotOverrides: Partial<SceneArborSnapshot> = {}): ProjectArborGate5SmokeEvidence {
  ...
  return {
    ...
    closeReopen: { iterations: [{ exactOnce: true }, { exactOnce: true }] },
    ...
    persistence: { version: 2, containsRuntimeOnlyIds: false },
    actorInputRouteEvidence: { hits: [{ partId: "fullscreen-toggle" }, { partId: "projection-mode" }] }
  };
}
